...
import { DollarSign, Clock, CheckCircle, XCircle, MessageCircle } from 'lucide-react';
import { getDriverNegotiations, acceptOffer, declineOffer, counterOffer } from '../lib/negotiation-service';
    ...
    return (
      <div className="bg-white rounded-xl shadow-md p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-bold text-gray-800">Price Negotiations</h3>
        </div>
        <p className="text-gray-500 text-sm">No pending negotiations at the moment.</p>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-xl shadow-md p-6">
      ...
      <div className="space-y-4">
        {negotiations.map((negotiation) => (
          <div key={negotiation.id} className="border border-gray-200 rounded-lg p-4 hover:border-blue-300 transition">
            {/* Customer Info */}
            <div className="flex items-start justify-between mb-3">
              <div>
                <h4 className="font-bold text-gray-800">{negotiation.customerName}</h4>
                <p className="text-sm text-gray-500">{negotiation.customerPhone}</p>
              </div>
              <div className="flex items-center gap-1 text-xs text-gray-500">
                <Clock className="w-3 h-3" />
                {new Date(negotiation.createdAt instanceof Date ? negotiation.createdAt : negotiation.createdAt.toDate()).toLocaleTimeString()}
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-3 p-3 bg-gray-50 rounded-lg">
              <div>
                ...
                <p className="text-lg font-bold text-gray-800">KES {negotiation.initialPrice.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Customer Offer</p>
                <p className="text-lg font-bold text-blue-600">KES {negotiation.proposedPrice.toLocaleString()}</p>
              </div>
            </div>
